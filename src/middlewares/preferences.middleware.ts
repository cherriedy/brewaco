...
/**
 * Express middleware to detect and set user preferences including locale, device, location, and date.
 *
...
export function preferencesMiddleware(
  ...
  req.device =
    req.headers["user-agent"] ?? t("preferences.unknownDevice", req.locale);
  ...
  if (req.ip) {
    ...
    req.location = t("preferences.unknownLocation", req.locale);
  }
